...
module.exports = async (bot, oldMember, newMember) => {
  if(newMember.roles.cache.some(r => botconfig.admin.some(role => r.id == role))){
    ...
    const logs = await guild.fetchAuditLogs({ type: "MEMBER_ROLE_UPDATE" }).then(async (audit) => {
      ...
      .setDescription(`Вы выдали пользователю ${newMember} роль Администратора. Для внесения в базу данных - следующим сообщением введите его STEAM_ID.\nУ вас есть 15 секунд.\nSTEAM_ID можно будет поменять командой \`!steam\``);
      ...
      collector.on('collect', async (m) => {
        ...
        if(!m.content.match(regexp)) {
          ...
          m.channel.send(`${m.author}, введенные вами данные не соответствуют STEAM_ID. Роли пользователя были сняты. Попробуйте вновь.`)
          return
        }
