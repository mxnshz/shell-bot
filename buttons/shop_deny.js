...
module.exports = async (bot, interaction) => {
  ...
  await interaction.channel.send({content: `**Пользователю <@${temp_db.DiscordID}> \`${user_db.SteamID}\` было отказано в выдаче ${temp_db.amount * botconfig.index} коинов за ${temp_db.amount} жалоб.\nЗапрос отклонил ${member_interaction}.**`})
  await bot.users.cache.get(temp_db.DiscordID)?.send({ content: `**Вам было отказано в выдаче ${temp_db.amount * botconfig.index} коинов за ${temp_db.amount} жалоб.\nЗапрос отклонил ${member_interaction}.**` }).catch(() => null)
