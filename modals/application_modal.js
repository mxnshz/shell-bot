...
module.exports = async (bot, interaction, args) => {
  ...
  if (buttonCooldown.has(interaction.user.id)) {
    await interaction.editReply({
      content: `На выполнение этого действия действует кулдаун 30 секунд, повторите попытку позже.`,
      ephemeral: true,
    });
  }
  ...
  await interaction.editReply({
    content: `Заявление будет рассмотрено в течение 3 дней.`,
    ephemeral: true,
  });
  ...
  let embed = new MessageEmbed()
    ...
    .addFields(
      ...
      {
        name: 'Ознакомились ли вы с правилами сервера?',
        value: `${interaction.fields.getTextInputValue('rules')}`,
      },
      {
        name: 'Сколько вы готовы уделять серверу?',
        value: `${interaction.fields.getTextInputValue('time')}`,
      },
      {
        name: 'Почему мы должны взять вас в администрацию?',
        value: `${interaction.fields.getTextInputValue('about')}`,
      }
    );
